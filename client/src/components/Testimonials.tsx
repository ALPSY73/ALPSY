      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {testimonials.map((testimonial, index) => (
            <div 
              key={index} 
              className={`${testimonial.name === "Alain" ? "bg-primary-blue" : testimonial.name === "Leïla" ? "bg-orange-500" : "bg-white"} p-6 rounded-xl shadow-lg border border-gray-100`}
            ...
            >
              <div className="flex items-center mb-4">
                ...
                <div>
                  <div className={`font-medium ${testimonial.name === "Alain" || testimonial.name === "Leïla" ? "text-white" : "text-dark-gray"}`} data-testid={`testimonial-name-${index}`}>
                    {testimonial.name}
                  </div>
              ...
              <p className={`${testimonial.name === "Alain" || testimonial.name === "Leïla" ? "text-white" : "text-gray-600"} italic text-justify`} data-testid={`testimonial-text-${index}`}>
                "{testimonial.text}"
              </p>
            </div>
          ))}
